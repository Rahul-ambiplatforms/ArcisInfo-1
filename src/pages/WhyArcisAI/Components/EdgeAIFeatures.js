...
const EdgeAIFeatures = ({ data }) => {
  ...
  return (
    <Box
      ...
      bgRepeat="no-repeat"
    >
      {/* Overlay for readability if needed, adjusting opacity as per design feel */}
      {/* <Box
        position="absolute"
        top="0"
        left="0"
        w="100%"
        h="100%"
        bg="rgba(255, 255, 255, 0.8)" // Light overlay to match the light grey aesthetic of the reference
        zIndex="0"
      /> */}
      ...
      <Box w="100%" position="relative" zIndex="1">
        ...
        <VStack spacing={6} textAlign="center" mb={16}>
          <Heading
            ...
            bgGradient="linear-gradient(91deg, #171717 19.68%, #7F56D9 100%)"
            bgClip="text"
            fontWeight="400"
        ...
        >
          {(() => {
            ...
            return gridItems.map((item, index) => {
              ...
              if (item.type === "text") {
                return <FeatureTextCard key={index} feature={item.feature} />;
              }
              return <Box key={index} />;
            });
          })()}
