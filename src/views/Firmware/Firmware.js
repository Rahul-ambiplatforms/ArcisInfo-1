...
const downloadFirmwareById = async (id, type) => {
  const res = await instance.get(`/firmware/download/${id}?type=${type}`, {
    responseType: 'blob',
    timeout: 60000,
  });
  let filename = type === 'releaseNotes' ? 'releaseNotes.txt' : 'firmwareFiles.zip';
...
const Firmware = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError('Failed to load firmware data. Please try again later.');
      } finally {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      ...
      return camera.includes(q) || version.includes(q) || model.includes(q);
    });
  }, [data, searchQuery]);
  ...
  const handleDownload = async (id, type) => {
    ...
    try {
      await downloadFirmwareById(id, type);
    } catch (err) {
      ...
      <Container maxW="1200px" px={{ base: 4, md: 8 }}>
        <VStack spacing={4} mb={{ base: 8, md: 12 }} textAlign="center" align="center">
          ...
          <Text color="gray.400" fontSize={{ base: 'md', md: 'lg' }} maxW="720px" lineHeight="1.7">
            Download the latest firmware for ArcisAI cameras and read the corresponding release notes
            to keep your surveillance system secure and up to date.
          </Text>
        </VStack>
        ...
        {!loading && !error && filteredData.length === 0 && (
          ...
          >
            ...
            {data.length === 0
              ? 'No firmware available yet. Please check back later.'
                ...
                <Table variant="unstyled" size="md">
                  ...
                  <Tbody>
                    {pageData.map((row) => {
                      ...
                      const fileKey = `${row._id}:firmware`;
                      return (
