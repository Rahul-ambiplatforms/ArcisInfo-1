...
const BlogsOverviewDash = ({ urlWords: urlWordsProp, initialBlog }) => {
  ...
  // f_auto,q_auto = best modern format + auto-quality; w_1920 caps width so
  // we don't ship the 5-10 MB originals.
  const IMAGE_BASE_URL =
  ...
  useEffect(() => {
    ...
    // If server provided an initialBlog, skip client fetch to ensure crawlers
    // received full HTML from the server. Otherwise fetch on the client.
    if (initialBlog) {
  ...
  const colors = ["#8266C9", "#000000", "#DB7B3A"];
  ...
  return (
    <Box mt="5%">
      {/* Moved 2026-09-16: the blog.content.schemas render that used to live here
          is now in app/blog/[slug]/page.js (the server component), where the data
          already is. The old call here was an unguarded .map(), so any post whose
          CMS payload omits `schemas` crashed the render; it also emitted schemaData
          without validating it. The server-side version guards the array and skips
          entries that are not parseable JSON. Removed here rather than duplicated,
          so the same JSON-LD is not emitted twice. */}
              ...
              <Text fontSize="16px" fontWeight="500" color="black">
                {(() => {
                  ...
                  return (
                    <>
                      <span>{label}</span>
                      <span style={{ color: "#8266C9", padding: "0 4px" }}>
                        ●
                      </span>
                      <span>
